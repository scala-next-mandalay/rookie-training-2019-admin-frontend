...
const valueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value").set;
    ...
    describe("testing category list", () => {
      ...
      it('testing delete button', () => {
        let categories = [{ id: 1, name: "a" }, { id: 2, name: "b" }];
        const deleteCategory = jest.fn((category) => {
          if (category.id === 1) categories = [{ id: 2, name: "b" }];
          else categories = [{ id: 1, name: "a" }];
        });
      ...
      it('testing cancle button', () => {
        ...
        const button = container.querySelectorAll('Button');
        //console.log('Delete ? :',button[2].textContent);
        act(() => {
          button[2].dispatchEvent(new MouseEvent('click', { bubbles: true }));
        });
        
        const buttonArr = document.querySelectorAll('Button');
        //console.log('All btns from testing Cancel :',buttonArr.length);
        //console.log('Clicked Cancel :',buttonArr[5].textContent);
